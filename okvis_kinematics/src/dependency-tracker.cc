    ...
    else//已经完成初始化
    {
		...
		//因为这两种情况之前都没有速度可以参考，所以无法使用匀速模型
                ...
                else
                {
                    ...
                    if(!bOK)//如果匀速模型模型失败则使用关键帧模型，与距离自己最近的关键帧的位姿=当前帧的初始位姿
                        bOK = TrackReferenceKeyFrame();
                }
            }
            else//跟踪失败，进行重定位!!!!!!!!!!!!!
          ...
          if(bOK)
        {
            ...
            //遍历上一帧中距离相机位姿最近的前100个地图点，实际程序中并没有使用mlpTemporalPoints这个变量
    ...
    if(mCurrentFrame.N>500)//如果当前帧的关键点数量大于500个
    {
        ...
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            ...
            if(z>0)
            {
                ...
                pNewMP->AddObservation(pKFini,i);//更新这个地图点的信息，被pKFini关键帧看到，并在pKFini关键帧中的
                pKFini->AddMapPoint(pNewMP,i);//更新关键帧中的地图点
                //观测到该地图点的多个特征点中（对应多个关键帧），挑选出区分度最高的描述子，作为这个地图点的描述子
                pNewMP->ComputeDistinctiveDescriptors();
                pNewMP->UpdateNormalAndDepth();//更新地图点的平均法线和深度信息
                mpMap->AddMapPoint(pNewMP);//向地图中添加地图点
                mCurrentFrame.mvpMapPoints[i]=pNewMP;//更新当前帧中的地图点
            }
        }
        ...
        mpMap->SetReferenceMapPoints(mvpLocalMapPoints);//更新地图中的特征点

        mpMap->mvpKeyFrameOrigins.push_back(pKFini);//更新地图中的关键帧
...
{
    //对于上一帧图像的特征点进行遍历，更新上一帧图像中特征点对应的地图点。
    //如果上一帧特征点对应的地图点被替换则更新上一帧对应的这个地图点。
    for(int i =0; i<mLastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            MapPoint* pRep = pMP->GetReplaced();//返回mpReplaced，这个变量只在Replace()函数中被更新
            if(pRep)
            {
...
void Tracking::UpdateLastFrame()
{
    ...
    //更新mLastFrame.mvpMapPoints的地图点，插入前100个距离相机位姿近的地图点
    int nPoints = 0;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    //将地图点投影到相机坐标系下的相机平面。优化变量只有pose，地图点位置固定，是一边元。
    ...
    if(mbOnlyTracking)//只运行tracking模式么澹?
    {
        mbVO = nmatchesMap<10;
        return nmatches>20;
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(nKFs<=2)
        nMinObs=2;
    int nRefMatches = mpReferenceKF->TrackedMapPoints(nMinObs);//返回高质量MapPoints的数量，被至少minObs个关键帧观测到
    ...
    const bool c1c =  mSensor!=System::MONOCULAR && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;//这里好像有问题吧System::MONOCULAR始终是0啊
...
{
    if(!mpLocalMapper->SetNotStop(true))//这个应该是让localmapping程序部分停止
        return;

    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB);//新建的关键帧

    mpReferenceKF = pKF;
    mCurrentFrame.mpReferenceKF = pKF;//设置当前帧的参考帧就是它自己
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        //这个程序好像之前出现过也是对特征点按照距离相机的大小进行排序然后选前100个，然后更新当前帧中的mvpMapPoints
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(bCreateNew)
                {
                    ...
                    MapPoint* pNewMP = new MapPoint(x3D,pKF,mpMap);//地图点
    ...
    mpLocalMapper->SetNotStop(false);//让localmapping的程序继续执行
...
void Tracking::SearchLocalPoints()
{
    ...
    //遍历局部地图中的地图点。判断是否在当前帧的视野范围内，如果在视野范围内则标记该点
   ...
   //轮询本地关键帧
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        for(vector<MapPoint*>::const_iterator itMP=vpMPs.begin(), itEndMP=vpMPs.end(); itMP!=itEndMP; itMP++)
        {
            ...
            if(!pMP)
                continue;
            if(pMP->mnTrackReferenceForFrame==mCurrentFrame.mnId)//参考帧是与当前帧中有最多相同特征点的帧
                continue;
            if(!pMP->isBad())
            {
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    map<KeyFrame*,int> keyframeCounter;//存储关键帧和共视次数
    //遍历当前帧的所有特征点找到共视这些特征点的其他关键帧
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
...
//使用epnp求解相机的初始位姿再使用非线性优化得到最终的位姿，epnp部分使用的是opencv中的代码
//已知世界坐标系空间点位置和图像对应像素坐标 求世界坐标系下的相机位姿
bool Tracking::Relocalization()
{
    ...
    {
        mnLastRelocFrameId = mCurrentFrame.mnId;//记录下上一次重定位的关键帧id
        return true;
    }
